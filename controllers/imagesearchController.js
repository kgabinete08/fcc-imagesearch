...
exports.search = async (req, res) => {
  ...
  const searchRequest = await axios.get(
    `https://www.googleapis.com/customsearch/v1?q=${searchTerm}&cx=${process.env
      .CSE_ID}&safe=medium&searchType=image&start=${offset}&fields=items(image(contextLink%2CthumbnailLink)%2Clink%2Csnippet)%2Curl&key=${process
      .env.SEARCH_KEY}`
  );
